...
TSON 最初作为 milkio 的一部分，后续为了使前端及其他项目可用而被分离出来，作为一个独立的 npm 包发布。
...
milkio 内置了 TSON，您发送的所有参数在接收并解析时，返回给前端的结果实际上都是通过 TSON 实现的。如果您正在使用 milkio，无需安装，直接通过以下方式使用即可。

milkio comes with TSON built-in. All the parameters you send will be received and parsed using TSON, and the results returned to the front-end are actually achieved through TSON. If you are using milkio, there is no need to install it. You can use it directly by following the steps below.
