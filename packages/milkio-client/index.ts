...
export const createMilkioClient = <ApiSchema extends ApiSchemaExtend, FailCode extends FailCodeExtend>(options: milkioClientOptions) => {
  ...
  const bootstrap = async () => {
    ...
    if (options.middlewares) {
      ...
      function _middlewareHanlder(index: number, options: MiddlewareOptions) {
    ...
    async execute<Path extends keyof ApiSchema["apiMethodsTypeSchema"], Params extends ExecuteParams<Path>>(path: Path, params: Params, headers?: Record<string, string>, executeOptions?: ExecuteOptions): Promise<ExecuteResult<Path>> {
      ...
      try {
        for (const m of _beforeExecuteMiddlewares) {
          await m.middleware(path as string, params, headers)
        }
        ...
        for (const m of _afterExecuteMiddlewares) {
          await m.middleware(path as string, result)
        }
...
export type BeforeExecuteMiddleware = (path: string, params: any, headers: Record<string, string>) => Promise<void> | void;
export type AfterExecuteMiddleware = (path: string, response: { value: any }) => Promise<void> | void;
